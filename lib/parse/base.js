...
var Base = module.exports = function (src, opts) {
	...
	if (opts.exclude) {
		if (opts.exclude instanceof RegExp || toString.call(opts.exclude) == '[object RegExp]') {
			//a single RegExp is passed
			this.excludeRegex = new RegExp(opts.exclude);
		} else {
			//we assume an array of RegExps is passed
			this.excludeRegex = [];
			var i;
			for (i = 0; i < opts.exclude.length; i += 1) {
				this.excludeRegex[i] = new RegExp(opts.exclude[i]);
			}
		}
	} else {
		this.excludeRegex = false;
	}
...
Base.prototype.isExcluded = function (id) {
	...
	if( Object.prototype.toString.call( this.excludeRegex ) === '[object Array]' ) {
		for (i = 0; i < this.excludeRegex.length; i += 1) {
			if (id.match(this.excludeRegex[i])) {
				return true;
			}
		}
		return false;
	} else {
		return id.match(this.excludeRegex);
	}
};
